...
const String _openWeatherApiKey = String.fromEnvironment('OPENWEATHER_API_KEY', defaultValue: '');

String get baseUrl {
  final fallback = _rawBaseUrl.trim().isEmpty ? 'http://127.0.0.1:5000' : _rawBaseUrl.trim();
...
class ApiService {
  ...
  static Future<Map<String, dynamic>> signup({
    ...
    print('Signup request => $baseUrl/signup, email=$email, role=$role');
  ...
  static Future<Map<String, dynamic>> login({
    ...
    print('Login request => $baseUrl/login, email=$email');
    final response = await _safeRequest(() {
      return http
          .post(
  ...
  static Future<Map<String, dynamic>> completeOnboarding({
    ...
    print('Onboarding request => $baseUrl/onboarding, city=$city, lat=$latitude, lon=$longitude');
    final response = await _safeRequest(() {
      return http
          .post(
  ...
  static Future<Map<String, dynamic>> claimPolicyNow({
    ...
    final payload = <String, dynamic>{'trigger_type': 'Weather Risk', 'lost_hours': 3};
  ...
  static Future<Map<String, dynamic>> adminOverview({String? department, String? category}) async {
    ...
    final uri = Uri.parse('$baseUrl/admin/overview').replace(queryParameters: query.isEmpty ? null : query);
  ...
  static Future<Map<String, dynamic>> weatherRisk({
    ...
    try {
      print('Weather URL: $weatherUri');
      print('Weather request source base URL: $baseUrl');
      final response = await http.get(weatherUri, headers: _headers()).timeout(const Duration(seconds: 10));
      print('Weather status: ${response.statusCode}');
      print('Weather body: ${response.body.length > 600 ? response.body.substring(0, 600) : response.body}');
      ...
      if (_token != null) {
        ...
        print('Weather fallback status: ${fallback.statusCode}');
        print('Weather fallback body: ${fallback.body.length > 600 ? fallback.body.substring(0, 600) : fallback.body}');
        if (fallback.statusCode >= 200 && fallback.statusCode < 300) {
          return _parse(fallback);
        }
      }

      print('Primary weather route failed, trying HTTPS OpenWeather direct fallback.');
      return await _weatherRiskFromOpenWeather(latitude: latitude, longitude: longitude);
    } on SocketException {
      print('Socket exception on backend weather route, trying HTTPS OpenWeather direct fallback.');
      return await _weatherRiskFromOpenWeather(latitude: latitude, longitude: longitude);
    } on TimeoutException {
      print('Timeout on backend weather route, trying HTTPS OpenWeather direct fallback.');
      return await _weatherRiskFromOpenWeather(latitude: latitude, longitude: longitude);
    } on FormatException {
  ...
  static Future<Map<String, dynamic>> _weatherRiskFromOpenWeather({
    ...
    if (_openWeatherApiKey.trim().isEmpty) {
      throw Exception('Unable to fetch weather data. Configure OPENWEATHER_API_KEY for secure fallback.');
    }
    ...
    print('OpenWeather HTTPS URL: $uri');
    final response = await http.get(uri, headers: _headers()).timeout(const Duration(seconds: 12));
    print('OpenWeather HTTPS status: ${response.statusCode}');
    print('OpenWeather HTTPS body: ${response.body.length > 600 ? response.body.substring(0, 600) : response.body}');
    ...
    final riskScore = (((rainProb / 100.0) * 0.45) + ((windSpeed / 20.0).clamp(0.0, 1.0) * 0.25) + (((10000 - visibility) / 10000.0).clamp(0.0, 1.0) * 0.30)).clamp(0.0, 1.0);
    ...
    return {
      ...
      'risk': {
        ...
        'recommendation': riskLabel == 'Low' ? 'Claim not eligible due to low risk' : (riskLabel == 'Medium' ? 'Claim approved (moderate risk)' : 'Claim approved (high risk)'),
      },
    };
  }
    ...
    final response = await _safeRequest(() {
      return http
          .put(
            Uri.parse('$baseUrl/profile'),
            headers: _headers(auth: true),
            body: jsonEncode({'name': name, 'city': city, 'location_text': locationText}),
          )
          .timeout(const Duration(seconds: 15));
    });
    return _parse(response);
  ...
  static Future<List<dynamic>> getClaims() async {
    ...
    if (response != null && response.statusCode >= 200 && response.statusCode < 300) {
      ...
      if (data is Map<String, dynamic> && data['claims'] is List) return data['claims'];
    }
  ...
  static Map<String, dynamic> _parse(http.Response response) {
    ...
    final message = body is Map<String, dynamic> ? (body['error'] ?? body['message'] ?? 'Request failed') : 'Request failed';
  ...
  static Future<http.Response> _safeRequest(Future<http.Response> Function() request) async {
    try {
      ...
      throw Exception('API server is unreachable at $baseUrl. Verify deployment status and API base URL.');
    } on TimeoutException {
      throw Exception('Server is taking too long to respond. Please try again shortly.');
    } catch (_) {
      rethrow;
    }
  }

  static Future<http.Response?> _safeRequestOrNull(Future<http.Response> Function() request) async {
    try {
      return await request();
    } on SocketException {
      return null;
